...
/// <summary>
/// Shared TCP print/scan/driver-package receiver (port 9877). Migrated from
/// <c>ShaPrint.WpfApp/Services/Server/PrintReceiver.cs</c> (Task 4) as an injectable class.
/// Windows-specific calls are routed through platform abstractions:
/// <list type="bullet">
/// <item><c>SpoolerApi.PrintRawDataAsync</c> → <see cref="IPrinterManager.PrintRawDataAsync"/></item>
/// <item><c>SpoolerApi.GetLocalPrintersDetailed</c> (driver-name resolution) → <see cref="IPrinterManager.GetLocalPrintersAsync"/></item>
/// <item><c>ScannerService</c> (WIA) → <see cref="IScannerService"/></item>
/// <item>driver package provisioning → <see cref="IDriverPackageProvider"/></item>
/// </list>
/// Wire protocol (PacketTypePrint/Scan/DriverPackage* handshake, AES-256-GCM payloads) is unchanged.
/// </summary>
public class PrintReceiverService
{
    ...
    public void SetDriverPackageProvider(IDriverPackageProvider? provider)
        => _driverPackageProvider = provider;

    public void SetDriverSharingEnabled(bool enabled)
        => _driverSharingEnabled = enabled;

    public PrintReceiverService(
        INotificationService notificationService,
        IPrinterManager printerManager,
        IScannerService scannerService,
        Action<JobHistoryEntry>? onJobLog = null,
        Action<ServerErrorEntry>? onErrorLog = null)
    {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrEmpty(payload.TargetPrinterName) && payload.SpoolData != null && payload.SpoolData.Length > 0)
                        {
                            ...
                            AppLogger.Log($"[SERVER] Injecting {payload.SpoolData.Length} bytes into print spooler for '{payload.TargetPrinterName}'");
                            bool printed = await _printerManager.PrintRawDataAsync(payload.TargetPrinterName, payload.SpoolData, docName);

                            if (printed)
                            {
                                AppLogger.Log($"[SERVER] SUCCESS: Print job accepted by spooler.");
                            ...
                            {
                                AppLogger.Error($"[SERVER] FAILED: Spooler rejected the job. Check printer manager logs.");
    ...
    private async Task HandleDriverPackageRequestAsync(NetworkStream stream, BinaryReader reader, string remoteIp, CancellationToken token)
    {
        try
        {
            if (!_driverSharingEnabled || _driverPackageProvider == null)
            {
            ...
            try
            {
                var allPrinters = await _printerManager.GetLocalPrintersAsync();
            ...
            var manifest = await _driverPackageProvider.GetDriverPackageAsync(driverName);
            if (manifest == null)
            {
            ...
            byte[]? packageBytes = await _driverPackageProvider.ReadPackageBytesAsync(manifest.Sha256);
