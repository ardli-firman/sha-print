...
/// <summary>
/// Shared UDP discovery server (port 9876). Migrated from
/// <c>ShaPrint.WpfApp/Services/Server/DiscoveryServer.cs</c> (Task 4) as an injectable class.
/// Windows-specific calls are routed through platform abstractions:
/// <list type="bullet">
/// <item><c>SpoolerApi.GetLocalPrintersDetailed()</c> → <see cref="IPrinterManager.GetLocalPrintersAsync"/></item>
/// <item><c>ScannerService</c> (WIA) → <see cref="IScannerService"/></item>
/// <item>driver package provisioning → <see cref="IDriverPackageProvider"/> (Windows wraps DriverPackageService)</item>
/// </list>
/// Network protocol, rate limiting, HMAC signing and client tracking are unchanged.
/// </summary>
    ...
    private async Task ListenLoopAsync(CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            try
            {
                ...
                var allDetailedPrinters = await _printerManager.GetLocalPrintersAsync();
                ...
                foreach (var p in _exposedPrinters)
                {
                    ...
                    if (!isMonitorRequest && _driverSharingEnabled && _driverPackageProvider != null)
                    {
                        try
                        {
                            var pkg = await _driverPackageProvider.GetDriverPackageAsync(driverName);
                            if (pkg != null)
                            {
                ...
                if (response.ExposedScanners != null)
                {
                    try
                    {
                        ...
                        foreach (var s in _exposedScanners)
                        {
                            ...
                            {
                                Name = s,
                                Description = found?.Description ?? "Scanner"
                            });
                        }
                    }
                    catch (Exception ex)
