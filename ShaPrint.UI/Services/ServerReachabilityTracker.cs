...
/// <summary>
/// Shared reachability tracker for installed virtual printers. Migrated from
/// <c>ShaPrint.WpfApp/Services/Client/ServerReachabilityTracker.cs</c> (Task 4). Depends on
/// <see cref="InstalledPrinterConfig"/> (now in ShaPrint.UI.Services) and
/// <c>DiscoveryResponseMessage</c> from Core — no OS or UI types.
///
/// NOT DI-registered: its collaborators are per-client delegates (config provider, discovery
/// scanner, identity-change callbacks) supplied by the ClientViewModel, mirroring how
/// ShaPrint.WpfApp builds it today.
/// </summary>
public sealed class ServerReachabilityTracker
{
    ...
    public async Task RequestRescanAsync(RescanReason reason, CancellationToken ct)
    {
        ...
        {
            AppLogger.Log($"[TRACKER] Rescan debounced. Reason: {reason}");
            return;
        }
        ...
        try
        {
            ...
            AppLogger.Log($"[TRACKER] Requesting rescan. Reason: {reason}");
